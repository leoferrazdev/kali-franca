...
import {
  ...
  validateCoverCopy,
} from '../lib/covers/cover-presets.ts';
...
test('define três posições de copy dentro da Safe Zone operacional', async () => {
  const source = await readFile(new URL('../lib/covers/cover-presets.ts', import.meta.url), 'utf8');
...
test('renderizador preserva o contrato visual e exporta blob', async () => {
  const source = await readFile(new URL('../lib/covers/render-cover.ts', import.meta.url), 'utf8');
...
test('centraliza contexto, headline e subtítulo na prévia e na exportação', async () => {
  ...
  const renderer = await readFile(new URL('../lib/covers/render-cover.ts', import.meta.url), 'utf8');
...
test('componente expõe o fluxo guiado e mantém a mídia local', async () => {
  const source = await readFile(new URL('../components/cover-studio.tsx', import.meta.url), 'utf8');
...
test('separa semanticamente navegação, identificação e título do cabeçalho', async () => {
  const route = await readFile(new URL('../app/capas/page.tsx', import.meta.url), 'utf8');
  const css = await readFile(new URL('../app/globals.css', import.meta.url), 'utf8');
...
test('propaga posição e mantém a guia Safe Zone fora da exportação', async () => {
  const component = await readFile(new URL('../components/cover-studio.tsx', import.meta.url), 'utf8');
  const renderer = await readFile(new URL('../lib/covers/render-cover.ts', import.meta.url), 'utf8');
